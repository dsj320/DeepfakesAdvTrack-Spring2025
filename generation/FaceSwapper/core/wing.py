...
if __name__ == "__main__":
    ...
    #cv2
    import cv2
    fan = FAN(fname_pretrained='/data/shuangjun.du/work/FaceSwap/FaceSwapper/checkpoints/wing.ckpt')  # 推荐指定预训练权重，否则是随机初始化的
    fan.eval()
    fan.cuda()

    # 设置输入输出路径
    path = '../data/CelebDF/CelebDF'
    save_path = '../data/CelebDF/CelebDF_lm_images'
    ...
    for file in (os.listdir(path)):
        cnt+=1
        print(f"processing {cnt} / {len(os.listdir(path))}")
        if file.endswith('.jpg') or file.endswith('.png'):
            img_path = os.path.join(path, file)
            img = Image.open(img_path).convert('RGB')
            img_tensor = preprocess(img).unsqueeze(0).cuda()  # (1, 3, 256, 256)

            # 生成lmk图
            landmark_figure_list = fan.get_landmark_curve(img_tensor)

            # 注意 landmark_figure 是 list，这里取第一个
            landmark_img = landmark_figure_list[0]  # (H,W,3), np.uint8
            # 生成mask
            #mask = landmark_img[:,:,0] > 0
            # 保存
            save_file = os.path.join(save_path, file)
            cv2.imwrite(save_file, cv2.cvtColor(landmark_img, cv2.COLOR_RGB2BGR))
            #mask_save_file = os.path.join(mask_save_path, file)
            #cv2.imwrite(mask_save_file, mask.astype(np.uint8))
