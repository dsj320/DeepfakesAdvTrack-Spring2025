...
class Solver(nn.Module):
    ...
    def load_pretrained_weights(self, ckpt_path):
        ...
        # 兼容不同保存格式
        if 'nets' in state_dict:
            state_dict = state_dict['nets']
        for name, module in self.nets.items():
            if name in state_dict:
                module.load_state_dict(state_dict[name], strict=False)
        for name, module in self.nets_ema.items():
            if name in state_dict:
                module.load_state_dict(state_dict[name], strict=False)
    ...
    def train(self, loaders):
        ...
        # 新增：优先加载自定义预训练模型
        if 'pretrained_ckpt' in config and config['pretrained_ckpt']:
            self.load_pretrained_weights(config['pretrained_ckpt'])
        elif config['resume_iter'] > 0:
    ...
    def test(self, loaders):
        ...
        print(f"Total images to test: {img_num}")
        f.close()
        ...
        for i in tqdm(range(0, total_iters)):
            ...
            #inputs长度
            print(inputs.src.shape)
            utils.disentangle_and_swapping_test(nets_ema, config, inputs, save_dir)
            print(f"Processed {i+1} 迭代，一共处理了 {(i+1)*config['batch_size']} 张图片")
