...
def disentangle_and_swapping_test(nets, config, inputs, save_dir):
    ...
    for i in range(len(srcid_taratt)):
        filename = result_first + src_name[i][0:-4]+'_to_'+ tar_name[i][0:-5]+'.png'
        filename_post = result_second + src_name[i][0:-4] + '_to_' + tar_name[i][0:-5] + '.png'
        filename_all  = result_third + src_name[i][0:-4] + '_to_' + tar_name[i][0:-5] + '.png'
